  ...
  LingsList() {
    return this.props.lings.lings.reverse().map((ling) => {
      if (this.props.lang === "Show All") {
        return (
          <div key={ling.id} className="timeline-hover">
              ...
              <Card className="ling mb-3">
                ...
                <CardFooter>
                  <Link to={`/reply/${ling.id}`}>
                    {
                      this.props.lings.replies
                        .filter((reply) => reply.parentId === ling.id)
                        .filter((reply) => reply.replyType === "reply").length
                    }{" "}
                    Replies
                  </Link>{" "}
                  |
                  <Link to={`/reply/${ling.id}`}>
                    {" "}
                    {
                      this.props.lings.replies
                        .filter((reply) => reply.parentId === ling.id)
                        .filter((reply) => reply.replyType === "correction")
                        .length
                    }{" "}
                    Corrections
                  </Link>
